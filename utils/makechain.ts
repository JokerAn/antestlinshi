...
// 通用品种
const CONDENSE_PROMPT = (res: any) =>
  `As an AI assistant,combine ` +
  res +
  ` given the following conversation and a follow up question, rephrase the follow up question to be a standalone question with chinese.
...
export const makeChain = (vectorstore: PineconeStore, pdfNameSpace?: String) => {
  ...
  const chain = ConversationalRetrievalQAChain.fromLLM(model, vectorstore.asRetriever(2), {
